...
#param diS{i in rings,j in S} = (0.5*(2*pi/mIntegral) * sum{k in Integral} cos(2*pi*(i/10)*cos(j*(ThetaS*pi/180)/samplesS)*cos(2*pi*k/mIntegral))); 
# Valeurs echantillonnee des di dans P
#param diP{i in rings,j in P} = (0.5*(2*pi/mIntegral) * sum{k in Integral} cos(2*pi*(i/10)*cos((ThetaP*pi/180) + j*(pi/2 - (ThetaP*pi/180))/samplesP)*cos(2*pi*k/mIntegral)));; 
...
subject to S1{s in S}: tau * sum{i in rings} (y1plus[i,s]+y1minus[i,s]) <= epsilon - sum{i in rings} (diS[i,s]*x[i]);
subject to S2{s in S}: tau * sum{i in rings} (y2plus[i,s]+y2minus[i,s]) <= epsilon + sum{i in rings} (diS[i,s]*x[i]);
subject to S3{p in P}: tau * sum{i in rings} (y3plus[i,p]+y3minus[i,p]) <= epsilon + 1 - sum{i in rings} (diP[i,p]*x[i]);
subject to S4{p in P}: tau * sum{i in rings} (y4plus[i,p]+y4minus[i,p]) <= epsilon - 1 + sum{i in rings} (diP[i,p]*x[i]);
